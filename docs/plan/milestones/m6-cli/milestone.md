...
```
cli/
âââ bin/agentbox              # Entry point, modular dispatcher
âââ lib/                      # Shared libraries (8 files)
â   âââ composefile.bash      # Compose file manipulation
â   âââ path.bash             # Path and file utilities
â   âââ select.bash           # User interaction helpers
â   âââ logging.bash          # Colored logging
â   âââ constants.bash        # Shared constants
â   âââ require.bash          # Dependency checking
â   âââ compat.bash           # Bash 3.2 compatibility shim
â   âââ run-compose           # Docker Compose wrapper
âââ libexec/                  # Command modules (6 modules, 12 scripts)
â   âââ init/                 # init, cli, devcontainer, policy
â   âââ edit/                 # edit, compose, policy
â   âââ bump/                 # bump, bump_service
â   âââ exec/                 # exec, _ (fallback)
â   âââ destroy/              # destroy
â   âââ version/              # version
âââ templates/                # Per-agent, per-mode templates
â   âââ policy.yaml
â   âââ claude/{cli,devcontainer}/
â   âââ copilot/{cli,devcontainer}/
âââ test/                     # BATS test suite (19 files)
```
...
The CLI is distributed as both a git clone (add `cli/bin` to PATH) and a Docker image (`ghcr.io/WiIIManidis/agent-sandbox-cli`). The Docker image eliminates local dependency requirements but trades off editor integration and startup speed.
...
- [x] Docker image (`ghcr.io/WiIIManidis/agent-sandbox-cli`)
